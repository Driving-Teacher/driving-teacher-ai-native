...
**[`docs/deck/ai-native.md`](docs/deck/ai-native.md)** — 처음부터 끝까지 한 흐름으로 이어지는 덱 하나입니다. 42장.

```
왜 → 도구를 고른다 → 내 손에 붙인다 → 남이 만든 걸 찾는다
   → 같이 배운다 → 터미널을 벗어난다 → 회사 색을 채운다 → 박는다
```
...
섹션이 8개입니다. 오늘 3섹션까지만 하고 다음에 이어도 되고, 특정 섹션만 띄워도 됩니다. 주소 끝에 `#22` 처럼 붙이면 그 장으로 바로 갑니다. **덱이 쪼개져 있을 필요는 없다는 뜻입니다.**
