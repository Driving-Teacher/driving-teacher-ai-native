...
**[`docs/deck/ai-native.md`](docs/deck/ai-native.md)** — 처음부터 끝까지 한 흐름으로 이어지는 덱 하나입니다. 49장. 노션 「🤖 AI Native」 페이지와 같은 뼈대입니다.

```
왜 (달라진 것 · 정의 · 가져오는 것) → 도구를 고른다 → 내 손에 붙인다
   → 남이 만든 걸 찾는다 → 같이 배운다 → 터미널을 벗어난다
   → 회사 색을 채운다 → 세 가지를 약속한다 → 한 줄을 박는다
```

### 왜 하나로 합쳤나

시즌1은 `week1~4` 네 파일로 쪼개져 있었습니다. 그래서 덱마다 「이번 주 숙제」, 「다음 주에 ○○를 합니다」로 끝났고, 계획이 바뀌면 그 예고가 거짓말이 됐습니다. 실제로 `week1` 은 "3주차에 OpenClaw" 라고 예고했는데 `week3` 은 Hermes 로 바뀌어 있었습니다.

**끊김의 원인이 주차 경계였습니다.** 그래서 경계를 없애고 하나로 꿨습니다. 「다음 주에」라는 말은 이 덱에 한 번도 나오지 않습니다.

### 한 번에 다 하지 않아도 됩니다

섹션이 9개입니다. 오늘 3섹션까지만 하고 다음에 이어도 되고, 특정 섹션만 띄워도 됩니다. 주소 끝에 `#22` 처럼 붙이면 그 장으로 바로 갑니다. **덱이 쪼개져 있을 필요는 없다는 뜻입니다.**

### 고치는 법

`.md` 만 고치고 아래를 돌립니다. `.html` 은 구운 결과물이라 직접 고치면 다음 빌드에 사라집니다.

```bash
python3 scripts/build-deck.py          # 굽기
python3 scripts/build-deck.py --check  # 최신인지만 확인
```

문법과 주의사항은 `docs/deck/ai-native.md` 맨 위 주석에 적어뒀습니다.
