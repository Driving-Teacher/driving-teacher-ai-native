...
6. **두 번째 체험** — 기존 파일을 읽고 수정하는 것도 보여준다:

```
이번엔 이미 있는 파일을 읽어볼게요.
이 프로젝트의 README.md를 읽고 요약해드릴게요.
```

- README.md를 읽어서 핵심 내용을 2~3줄로 요약
- 그 다음: "이번엔 파일을 수정해볼게요. README.md에 여러분 이름을 추가해볼까요?"
- 사용자 이름을 물어보고, README.md의 적절한 위치에 추가
- **핵심 메시지**: "ChatGPT는 답변을 복사해야 하지만, Claude Code는 파일을 직접 읽고, 만들고, 수정합니다. 이게 'AI가 출근한 것'의 의미예요."
...
2. 스킬을 만들기 전에, `/think-deeper`로 먼저 정리한다:

```
스킬을 만들기 전에, 뭘 자동화할지 정리해봅시다.
/think-deeper를 써볼게요 — 발표에서 보여드린 그 스킬입니다.
```

- `/think-deeper`를 실행해서 "어떤 업무를 자동화하면 좋을지" 주제로 진행
- 4단계 전부 하면 시간이 오래 걸리므로, **Step 1 (문제 정의)만** 빠르게 해본다
- "왜 이 업무가 귀찮은지" 근본 원인을 1~2단계만 파고 넘어간다
- 결과를 바탕으로 아래 스킬 선택으로 연결
