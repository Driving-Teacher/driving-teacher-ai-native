...
### 데모 1: Skill 실행 — `/think-deeper`

> 강사가 직접 시연한다. 참가자는 보기만 한다.

강사가 터미널에서 아래를 입력하고, Claude가 단계별 질문 → 5 Whys → Plan A/B/C → 임기응변 체크까지 구조화된 기획을 진행하는 과정을 보여준다:

```
/think-deeper
```

핵심 포인트: **한 마디면 끝**. 모호한 기획이 구조화된 문서로 자동 생성된다. 7일 후에는 여러분도 이런 걸 직접 만들 수 있다.
